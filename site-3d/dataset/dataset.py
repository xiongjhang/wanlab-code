...
import siteflow.dataset.augment as transforms
from siteflow.dataset.dataload import get_slice_builder
from siteflow.dataset.utils import create_padded_indexes, traverse_h5_paths, default_prediction_collate, calculate_stats, mirror_pad
from siteflow.siteflow.utils.util import get_logger
    ...
    def __getitem__(self, idx):
        ...
        if self.phase == 'test':
            if len(raw_idx) == 4:
                ...
                raw_idx_padded = (slice(None),) + create_padded_indexes(raw_idx, self.halo_shape)
            else:
                raw_idx_padded = create_padded_indexes(raw_idx, self.halo_shape)
...
if __name__ == "__main__":
    ...
    file_path = '/mnt/sda/xjh/dataset/3dunet/3DUnet_lightsheet_nuclei/data/t00015_s01_uint8_cropped_gt.h5'
    with h5py.File(file_path, 'r') as f:
        raw = f['raw'][:]
        stats = calculate_stats(raw)
